            ...
            switch (messages.First())
            {
                case CatalogLeafScanMessage clsm:
                    var catalogLeafScan = await _catalogScanStorageService.GetLeafScanAsync(
                        ...
                        clsm.PageId,
                        clsm.LeafId);
                    if (catalogLeafScan.ParsedDriverType == CatalogScanDriverType.NuGetPackageExplorerToCsv)
                    {
                        batchSize = 1;
                    }
                    else if (catalogLeafScan.ParsedDriverType == CatalogScanDriverType.PackageAssemblyToCsv)
                    {
                        batchSize = 10;
                    }
                    else
